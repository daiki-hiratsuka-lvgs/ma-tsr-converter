...
## step8 不明の Salesforce 照合（厳密特定＋M&A候補）

    node sf_recover.js [SalesForceのCSV]

不明（法人番号なし）を、SF 既存レコードが持つ **電話・代表者・郵便・設立年月・法人番号** と突合して特定する。国税庁/gBiz は社名＋住所しか持たないが、SF はこれらを保持しているため拾える。

- **同一市区町村**を必須（都道府県・市区町村が違えば別会社/関連会社の恐れ）
- 社名は必須にしない → **設立年月**（不変・グループ各社で異なる）をアンカーに改称/買収へ対応
- 電話や代表者「だけ」の一致は不採用（グループ代表電話の共用・同一代表者の別会社を排除）
